...
- Serve the preview with the same `Cross-Origin-Embedder-Policy: require-corp` asserted by Bolt's parent document, plus `Cross-Origin-Resource-Policy: same-origin`, so Firefox can embed it without relaxing the iframe's opaque-origin sandbox.
- Run the preview in an iframe sandbox that allows only outbound popup navigation. Scripts and forms remain forbidden, and the rendered document's CSP independently blocks scripts.
